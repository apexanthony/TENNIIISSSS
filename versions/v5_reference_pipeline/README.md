# V5 Reference-Style Pipeline

V5 is an integrated event-analysis pipeline inspired by the reference project.
...
Previous V4 bounce-only logic was too dependent on trajectory shape. It could confuse a player hit with a bounce. V5 adds player context, so trajectory changes near the player are treated as hit candidates first, while trajectory changes away from the player remain bounce candidates.
...
```powershell
python versions\v5_reference_pipeline\infer_video_v5_pipeline.py ^
  --auto-product-mode ^
  --model-path .\exps\lite_heatmap_v3_270x480_from240\model_best_thr070_pw15.pt ^
  --video-path .\input.mp4 ^
  --video-out-path .\exps\v5_reference_pipeline\auto_v5.mp4 ^
  --track-csv-out .\exps\v5_reference_pipeline\auto_v5_track.csv ^
  --event-csv-out .\exps\v5_reference_pipeline\auto_v5_events.csv ^
  --input-height 270 ^
  --input-width 480 ^
  --threshold 0.30 ^
  --top-k 5 ^
...
python versions\v5_reference_pipeline\infer_video_v5_pipeline.py ^
  ...
  --model-path .\exps\lite_heatmap_v3_270x480_from240\model_best_thr070_pw15.pt ^
  --video-path .\input.mp4 ^
  --video-out-path .\exps\v5_reference_pipeline\reference_bounce.mp4 ^
  --track-csv-out .\exps\v5_reference_pipeline\reference_bounce_track.csv ^
  --event-csv-out .\exps\v5_reference_pipeline\reference_bounce_events.csv ^
  --input-height 270 ^
  --input-width 480 ^
  --threshold 0.30 ^
  --top-k 5 ^
...
Apply reference-style auxiliary modules to an existing V3/V5 track CSV:

```powershell
python versions\v5_reference_pipeline\apply_reference_assist.py ^
  --auto-product-mode ^
  --event-mode reference_bounce ^
  --video-path .\input.mp4 ^
  --track-csv-in .\exps\v5_reference_pipeline\input_track.csv ^
  --track-csv-out .\exps\v5_reference_pipeline\reference_assist_track.csv ^
  --event-csv-out .\exps\v5_reference_pipeline\reference_assist_events.csv ^
  --video-out-path .\exps\v5_reference_pipeline\reference_assist.mp4 ^
  --bounce-threshold 0.72 ^
  --min-event-gap 18 ^
  --min-flight-frames 6 ^
  --max-bounce-step 260 ^
...
python versions\v5_reference_pipeline\infer_video_v5_pipeline.py ^
  ...
  --video-out-path .\exps\v5_reference_pipeline\demo_v5.mp4 ^
  --track-csv-out .\exps\v5_reference_pipeline\demo_v5_track.csv ^
  --event-csv-out .\exps\v5_reference_pipeline\demo_v5_events.csv ^
  --input-height 270 ^
  --input-width 480 ^
  --threshold 0.30 ^
  --top-k 5 ^
...
python versions\v5_reference_pipeline\infer_video_v5_pipeline.py ... --use-mediapipe
```
...
python tools\debug_mediapipe_crops.py ^
  ...
  --out-dir .\exps\v5_reference_pipeline\mediapipe_debug
```
...
python versions\v5_reference_pipeline\infer_video_v5_pipeline.py ^
  --model-path .\exps\lite_heatmap_v3_270x480_from240\model_best_thr070_pw15.pt ^
  --video-path .\input.mp4 ^
  --video-out-path .\exps\v5_reference_pipeline\demo_v5_context.mp4 ^
  --track-csv-out .\exps\v5_reference_pipeline\demo_v5_context_track.csv ^
  --event-csv-out .\exps\v5_reference_pipeline\demo_v5_context_events.csv ^
...
When `--annotations-csv --game --clip` are provided, V5 also reads `court_1...court_14` and maps image coordinates to normalized mini-court coordinates.

## Outputs

```text
track CSV:
  frame,x,y,score,raw_x,raw_y,raw_score,shape_quality,hough_quality,court_x,court_y,accepted,source,gap

event CSV:
  frame,event_type,x,y,score,near_player,accel,angle_change,source
...
This is a V5 prototype. It is intended for rapid validation of the reference-style idea:
...
15. CSV adapter for applying reference-style auxiliary modules to existing V3/V5 tracks.
