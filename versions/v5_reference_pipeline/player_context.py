...
def empty_player_context() -> PlayerFrameContext:
    return PlayerFrameContext(hands=[], boxes=[], centers=[], hit_radius=0.0, crop_boxes=[])
...
def context_from_boxes(boxes: Sequence[BBox]) -> PlayerFrameContext:
    ...
    return PlayerFrameContext(hands=hands, boxes=list(boxes), centers=centers, hit_radius=radius, crop_boxes=[])
...
class MediaPipePoseProvider:
    def __init__(self, width: int, height: int, model_complexity: int = 1, crop_config: str = ""):
        ...
        self.pose_top = solutions.pose.Pose(
            model_complexity=model_complexity,
            min_detection_confidence=0.35,
            min_tracking_confidence=0.35,
        )
        self.pose_bottom = solutions.pose.Pose(
            model_complexity=model_complexity,
            min_detection_confidence=0.35,
            min_tracking_confidence=0.35,
        )
    ...
    def _process_crop(self, frame, crop, pose) -> PlayerFrameContext:
        ...
        if box_width < crop_width * 0.025 or box_height < crop_height * 0.08:
            return empty_player_context()
        return PlayerFrameContext(hands=hands, boxes=[box], centers=[center], hit_radius=radius, crop_boxes=[crop])

    def process(self, frame) -> PlayerFrameContext:
        poses = [self.pose_top, self.pose_bottom]
        contexts = [self._process_crop(frame, crop, poses[idx % len(poses)]) for idx, crop in enumerate(self.crops)]
        ...
        return PlayerFrameContext(hands=hands, boxes=boxes, centers=centers, hit_radius=radius, crop_boxes=crop_boxes)
