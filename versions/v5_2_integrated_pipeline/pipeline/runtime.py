...
from court import (  # noqa: E402
    ...
    transform_point,
    stabilize_homography_update,
)
from court import point_in_polygons  # noqa: E402
from detector import detect_events, seconds_to_frames, write_event_csv  # noqa: E402
from heatmap_candidates import (  # noqa: E402
    choose_candidate,
    distance_xy,
...
def build_auto_court_maps(args):
    ...
    maps = CourtMapSequence()
    frame_id = 0
    last_matrix = None
    smoothed_corners = None
    last_detection_quality = 0.0
    frames_since_detection = 10**9
    try:
        while True:
            ok, frame = cap.read()
            if not ok:
            ...
            if frame_id % max(1, args.court_detect_stride) == 0:
                ...
                if matrix is not None:
                    smoothed_corners, stable_matrix, quality = stabilize_homography_update(
                        smoothed_corners,
                        matrix,
                        frame.shape[1],
                        frame.shape[0],
                    ...
                    if stable_matrix is not None:
                        last_matrix = stable_matrix
                        last_detection_quality = quality
                        frames_since_detection = 0
            if last_matrix is not None:
                maps[frame_id] = last_matrix
                decay = max(0.55, 1.0 - 0.012 * frames_since_detection)
                maps.qualities[frame_id] = last_detection_quality * decay
...
def build_player_contexts(args, width: int, height: int) -> Dict[int, PlayerFrameContext]:
    cache_value = getattr(args, "player_context_cache", "")
    ...
    if not args.use_mediapipe:
        stabilized = stabilize_player_contexts(contexts, width, height)
        if cache_path is not None:
            cache_path.parent.mkdir(parents=True, exist_ok=True)
            with open(cache_path, "wb") as fp:
                pickle.dump(stabilized, fp)
        return stabilized
...
def nearest_court_quality(court_maps, frame_id: int, max_gap: int = 2):
    if frame_id in court_maps:
        return court_map_quality(court_maps, frame_id)
    for gap in range(1, max_gap + 1):
        if frame_id - gap in court_maps:
            return court_map_quality(court_maps, frame_id - gap)
        if frame_id + gap in court_maps:
            return court_map_quality(court_maps, frame_id + gap)
    return 0.0
...
def write_track_csv(path: str, stable, court_maps=None) -> None:
    ...
    with open(path, "w", newline="", encoding="utf-8") as fp:
        ...
        for frame_id, point in enumerate(stable["tracks"]):
            ...
            court_quality = nearest_court_quality(court_maps, frame_id)
            writer.writerow(
                [
                    frame_id,
...
def _scale_mini_point(point, origin_x, origin_y, court_w, court_h):
    x = origin_x + int(max(0.0, min(1000.0, point[0])) / 1000.0 * court_w)
    y = origin_y + int(max(0.0, min(2168.0, point[1])) / 2168.0 * court_h)
    return x, y


def _draw_reference_mini_court(frame, court_trace, ctx, event_by_frame, frame_id, court_maps):
    height, width = frame.shape[:2]
    panel_w = max(150, min(230, int(width * 0.14)))
    panel_h = int(panel_w * 2.17)
    margin = max(12, int(width * 0.012))
    x0 = width - panel_w - margin
    y0 = margin
    ...
    matrix = nearest_court_map(court_maps, frame_id)
    for center in ctx.centers:
        court_center = transform_point(matrix, center)
        if court_center is not None:
            cv2.circle(frame, _scale_mini_point(court_center, x0, y0, panel_w, panel_h), 4, (80, 180, 255), -1)

    points = [pt for pt in court_trace if pt is not None]
    for age, pt in enumerate(reversed(points)):
        radius = 4 if age == 0 else max(1, 3 - age // 5)
        color = (0, 0, 255) if age == 0 else (0, 220, 255)
        cv2.circle(frame, _scale_mini_point(pt, x0, y0, panel_w, panel_h), radius, color, -1, cv2.LINE_AA)

    for event in event_by_frame.get(frame_id, []):
        court_event = transform_point(matrix, (event.x, event.y))
        if court_event is None:
            continue
        color = (0, 80, 255) if event.event_type == "hit" else (255, 255, 0)
        cv2.circle(frame, _scale_mini_point(court_event, x0, y0, panel_w, panel_h), 7, color, 2)


def _draw_reference_main_frame(frame, trace, point, raw, events, args):
    points = [pt for pt in trace if pt is not None]
    for age, pt in enumerate(reversed(points)):
        radius = max(args.trace_min_radius, args.trace_radius - age // max(1, args.trace_decay))
        color = (0, 0, 255) if age == 0 else (0, 220, 255)
        cv2.circle(frame, (int(pt[0]), int(pt[1])), radius, color, -1, cv2.LINE_AA)
    if point is not None:
        cv2.circle(frame, (int(point[0]), int(point[1])), max(5, args.trace_radius + 2), (0, 0, 255), -1, cv2.LINE_AA)
        cv2.circle(frame, (int(point[0]), int(point[1])), max(8, args.trace_radius + 5), (255, 255, 255), 1, cv2.LINE_AA)
    if args.draw_raw and raw is not None:
        cv2.circle(frame, (int(raw[0]), int(raw[1])), 5, (255, 180, 0), 1, cv2.LINE_AA)

    for event in events:
        color = (0, 80, 255) if event.event_type == "hit" else (255, 255, 0)
        label = "H" if event.event_type == "hit" else "B"
...
def draw_outputs(args, stable, events, player_contexts, court_polygons, court_maps=None) -> None:
    court_maps = court_maps or {}
    event_by_frame = {}
    for event in events:
    ...
    court_trace = deque(maxlen=args.trace)
    frame_id = 0
    try:
        while True:
            ok, frame = cap.read()
            ...
            ctx = nearest_context(player_contexts, frame_id)
            if args.draw_court and args.render_style != "reference":
                draw_polygons(frame, court_polygons)
            if args.draw_players and args.render_style != "reference":
                draw_player_context(frame, ctx)

            point = stable["tracks"][frame_id]
            raw = stable["raw_tracks"][frame_id]
            trace.append(point)
            court_point = transform_point(nearest_court_map(court_maps, frame_id), point)
            court_trace.append(court_point)

            if args.render_style == "reference":
                _draw_reference_main_frame(frame, trace, point, raw, event_by_frame.get(frame_id, []), args)
                if args.draw_mini_court:
                    _draw_reference_mini_court(frame, court_trace, ctx, event_by_frame, frame_id, court_maps)
            else:
                if args.draw_raw and raw is not None:
...
def parse_args():
    parser = argparse.ArgumentParser(description="V5.1 reference-style TrackNet + player/court event pipeline.")
    parser.add_argument(
        "--auto-product-mode",
        action="store_true",
...
def main() -> None:
    ...
    print(
        "v51_mode: auto_product_mode={}, mediapipe={}, auto_court_lines={}, annotations={}".format(
